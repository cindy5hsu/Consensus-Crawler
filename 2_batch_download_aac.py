...
def worker(task_queue, results):
    ...
    while True:
        try:
            ...
            if task is None:
                task_queue.task_done()  # 標記 None 任務為完成
                break
...
def main():
    ...
    try:
        with open(csv_file, 'r', encoding='utf-8') as file:
            ...
            if not all(field in csv_reader.fieldnames for field in ["\ufeffMeeting", "m3u8"]):
                safe_print("[!] The CSV file is missing required columns. It should have '\ufeffMeeting' and 'm3u8' columns.")
                return
            
            # Queue all download tasks
            for row in csv_reader:
                file_name = row["\ufeffMeeting"]
                m3u8_url = row["m3u8"]
